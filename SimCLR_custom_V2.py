...
train_set = CustomDataset(root_dir='./datasetMestradoGledson', transform=transform)
...
class SimCLRTransform:
    def __init__(self, s=0.5):
        ...
        self.color_distortion = transforms.Compose([
            ...
            transforms.GaussianBlur(kernel_size=3)  # Added Gaussian blur as per SimCLR strategy
        ])
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.backbone = torchvision.models.resnet18(pretrained=True)  # Set pretrained=True if using pretrained weights
...
test_set = CustomDataset(root_dir='./datasetMestradoGledsonTest', transform=transform)
...
with torch.no_grad():
    for images, labels in test_loader:  # Use the same test loader as SimCLR for fair comparison
        images = images.to(device)
        outputs = baseline_model(images)
        _, predicted = torch.max(outputs, 1)
        all_labels.extend(labels.cpu().numpy())
